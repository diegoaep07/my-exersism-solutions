...
export function buildBirthdaySign(age) {
  const young_or_old = age >= 50 ? 'mature' : 'young'

  return `Happy Birthday! What a ${young_or_old} fellow you are.`
}
...
export function costOf(sign, currency) {
  const SIGN_PRICE = 20 + (sign.length * 2)
  return `Your sign costs ${SIGN_PRICE.toFixed(2)} ${currency}.`
}
